...
| Per-account FPS limiter | Per-row FPS-cap dropdown writing Roblox's settings pre-launch; Bloxstrap-conflict warning. Since v1.13 the cap survives close-together launches: Roblox keeps one shared settings file, so RoRoRo holds the next write until the previously launched client has provably read its own (measured — a client's first write-back proves the read). ~15-20s per account when caps differ, free when they match, with a dismissible banner. **Confirmed on screen 2026-08-04** — three accounts set to 20 / 60 / 240, launched back to back, each client's own cap read off Roblox's Shift+F5 performance overlay. That is the first recorded PASS: the previous measurement (2026-08-02, pre-fix) had every client running the NEXT account's value. Re-run via `docs/smoke-2026-08-04-fps-cap-regression.md`. | v1.2 line; survives multi-launch v1.13 | `src/ROROROblox.Core/FpsPresets.cs`, `FpsCapSettler.cs`, `GlobalBasicSettingsProbe.cs` |
...
- ~~Discord integration~~ — **SHIPPED in v1.15.0.0.** Retained here only for the platform limit, which still applies: Two halves, both off by default. **Rich presence + Join:** roster-level status over the local Discord pipe, with a Join button that works for public and private servers. **Per-account alerts:** an account dropping out unexpectedly, and a client crossing the memory threshold — routed per-trigger to a desktop notification, your own Discord channel, or a clan channel, muted per-account, coalesced per sweep, on a per-(account, kind) cooldown. Setup is the accessibility work: the paste field names what you actually pasted, a valid webhook reports which channel it posts to, Send test uses the real path, and there is a walkthrough for people who have never owned a Discord server. **Known platform limit:** while Roblox is running, Discord shows Roblox to your friends instead of RoRoRo — detection beats an RPC app for the profile slot, and detection is earned by installs over time, not submitted. Stated in Settings and in `PRIVACY.md`.
- ~~Naming consistency pass~~ — **SHIPPED in v1.15.0.0.** the product introduced itself as `ROROROblox.App` in the exe version resource (Task Manager, UAC, Explorer, Discord's "Add a game" list), as `RORORO` in the MSIX `DisplayName` and the Velopack installer title, and named the wrong form of itself in the manifest's Roblox trademark disclaimer. All corrected to **RoRoRo**; Store listing artwork regenerated to match. Two identity values deliberately unchanged because they are not display strings: Velopack `--packId` (`RORORO` — the update identity; changing it strands every existing install) and `PublisherDisplayName` (`626Labs LLC`, no space — must match Partner Center byte for byte). **Still manual at submission:** the Store listing title in Partner Center, which becomes *"RoRoRo — Multi-launcher for Windows"* rather than the bare wordmark, since "RoRoRo" alone is a coined word with no search volume.
...
- **Preferences restructure + move streamer mode into it** (top of backlog, 2026-08-04) — the Preferences window now carries startup, idle, Discord, alerts, and theme in one scrolling page; the header said "Startup" until tonight. Wants sections (left nav preferred) rather than a longer scroll. Streamer mode currently lives on the main window and belongs in Settings with everything else — do both in one pass, since moving it is a header/navigation decision as much as a control move.
- **Clan status channel — one live-updating card per member** — a webhook post can be edited in place (`PATCH /webhooks/{id}/{token}/messages/{id}`; POST with `?wait=true` returns the message id), so a status card can update rather than repost. One pinned message per member showing accounts running, uptime, which game, how many together, peak memory, last recycle. Turns a clan channel into a roster board instead of a log. Wants its own spec — related to the approved-but-unbuilt account-stats work above.
- **One shared clan webhook with per-member display names** — Discord caps webhooks at **10 per channel**, so "one webhook per clan member" breaks at the eleventh person. A webhook post can override `username` per message, so one shared URL plus each member's own name is uncapped and easier to hand out. Tradeoff: everyone holds a credential that can post to that channel until it is rotated.
- **Two tests flake on CI with the same shape: real wall-clock budgets** (2026-08-04). `FpsCapSettlerTests.PostWriteQuietWait_CompetingWriteLandsInsideTheWindow_ForcesARetry` failed on both v1.15 feature PRs; `RobloxProcessTrackerTests.InstallerRunning_ExtendsDeadline_AttachesLateRpb_NoAttachFailed` failed the v1.15 release run and passed 3/3 locally straight after. The second one budgets a 1-second `installerExtendedTimeout` with a 50ms poll and a 2-second `WaitAsync` — tight enough that a loaded runner is a coin flip. PR #80's attempt to fix the first did not hold, and three patches across two sessions say the problem is the design, not the constant: **do not raise the timeouts.** Treat this as one investigation into wall-clock-dependent tests, not two bug reports. Both are test-harness problems — the FPS feature itself was measured working on screen 2026-08-04.
...
- **Plugin `RequestLaunchTarget` can now block ~20s** — `MainViewModelLaunchInvokerAdapter.cs:110` awaits the launch, so the gRPC handler at `PluginHostService.cs:385` holds open for up to `FpsCapSettler.SettleTimeout`, the settle ceiling that replaced the old launch-gate's ~30s bound. If RoRoRo Ur Task sets a client-side deadline under 20s, plugin-triggered launches will report `DEADLINE_EXCEEDED` while the launch actually succeeds. Needs one grep in the Ur Task repo before the next plugin release.
