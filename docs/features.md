...
| Discord Join, outbound and inbound | Pipe-delimited join secret (<128 chars); inbound via the in-client Join button or the `roblox-rororo:` URI (cold start or single-instance relay); confirm keyed on origin. Unpackaged builds register the scheme in HKCU; packaged builds get it from the manifest's `uap10:Protocol` entries, which deliver the URI as the same argv (v1.24 — before that the packaged path was inert: HKCU writes virtualized, verified live 2026-08-30). | `Core/Discord/JoinSecretCodec.cs`, `App/Discord/InboundJoinDispatcher.cs`, `App/Discord/JoinUriScheme.cs`, `App/Modals/JoinRequestWindow.xaml.cs`, `App/Package.appxmanifest` | `PresenceEnabled && JoinEnabled` | active |
| Alerts: dropped out, memory warning, recycled, uptime marks | Pure router (per-account mute, 5-min cooldown per (account, kind), coalescing, desktop fallback) then tray toast or webhook POST; the clan destination gets real names. | `Core/Discord/AlertRouter.cs`, `App/Discord/AlertDispatcher.cs`, `App/Discord/DiscordWebhookSender.cs`, `Core/Discord/WebhookPayload.cs` | destinations in `discord.dat` (default None); independent of the app id | active |
...
| Phone alerts (Pushover / ntfy) | `AlertDestination.Phone` on the same router (mute/cooldown/desktop-fallback for free); Pushover with drop=priority-1, ntfy with random bearer topic and static ASCII title; credential rejection is terminal per session like a webhook 404. Discord webhooks deliberately untouched (2026-09-04 ruling). | `Core/Notify/PhoneNotifyConfig.cs`, `App/Notify/PushoverSender.cs`, `App/Notify/NtfySender.cs`, `App/Notify/PhoneAlertSender.cs`, `Core/Discord/AlertRouter.cs` | destinations in `discord.dat`; provider + credentials in `notify.dat` (default off) | active (live smoke PASS 2026-09-05, spec §4 — both providers + a real alt-drop page) |
| Phone setup UX | Provider picker, F-076 masked/reveal credential fields, Pushover paste validation, crypto-random ntfy topic generation with re-subscribe warning, Test my phone, status-line integration. | `App/Preferences/SettingsPage.xaml(.cs)`, `Core/Notify/PhoneCredentialValidator.cs`, `Core/Notify/NtfyTopicGenerator.cs`, `Core/Discord/AlertStatusLine.cs` | Settings › Alerts | active (live smoke PASS 2026-09-05, spec §4) |
...
| Metric alerts (`ReportMetric`) | A consented plugin reports one raw, cumulative number per (account, metric id); the host keeps a bounded per-series history, derives rates itself, applies Rate/Level/Event rules, and raises a `MetricBreach` through the same router (mute, cooldown, coalescing, streamer masking) every other alert kind uses. Future-dated observations are dropped and logged, not clamped. | `src/ROROROblox.PluginContract/Protos/plugin_contract.proto` (`ReportMetric`), `App/Plugins/PluginHostService.cs`, `App/Plugins/Adapters/MetricReportSinkAdapter.cs`, `Core/Metrics/MetricAlertCoordinator.cs`, `Core/Metrics/MetricHistory.cs`, `Core/Metrics/MetricEvaluator.cs`, `App/Metrics/LocalFileMetricRuleSource.cs`, `App/Preferences/SettingsPage.xaml(.cs)`, `Core/Discord/AlertRouter.cs`, `App/Discord/AlertDispatcher.cs`, `docs/plugins/AUTHOR_GUIDE.md` | `host.metrics.report` capability; `settings:MetricAlertsEnabled` (false, toggled in Settings › Alerts), read per report; destinations in `discord.dat` (default `Local`) | active — report → rule → alert reaches desktop, personal Discord, clan Discord, and phone, all four resolved through the same `AlertRouter`/`AlertDispatcher` every other alert kind uses, unconfigured-destination fallback included. The **Metric alerts** toggle and its four destination checkboxes in Settings › Alerts write `MetricAlertsEnabled` and `MetricBreachDestinations`. Rule source is `%LOCALAPPDATA%\ROROROblox\metric-rules.json` (unsigned, hand-edited) — this is the rule source, permanently: a signed 626-hosted manifest was designed to replace it and was dropped 2026-09-11 (`docs/superpowers/specs/2026-09-09-external-metric-alerts-design.md`), and the file stays unsigned because a rule names a metric id, a kind, a threshold and a window, never a URL, so there is nothing here that signing would protect. Nothing on this row has been smoke-tested on a real machine |
...
| Auto-update (Velopack) | Once per 24 h via `GithubSource(prerelease:false)`; a found update is downloaded and staged with `WaitExitThenApplyUpdates` (silent, no restart) — it applies when the app exits, and the next launch is the new version. | `App/Updates/UpdateChecker.cs` | Velopack `IsInstalled` (dev and packaged builds skip) | wired 2026-09-05 (build-plan item 11; check-only from v1.0 through v1.25) — a user must run one wired version before the promise holds for them |
...
| Run on login | Unpackaged: HKCU `Run` value `RORORO` = exe path. Packaged: manifest `StartupTask` `RoRoRo` via `Windows.ApplicationModel.StartupTask` (v1.24 — before that the HKCU write landed in the package's virtual hive and the toggle read its own dead value back, verified live 2026-08-30). | `App/Startup/StartupRegistration.cs`, `App/Startup/PackagedStartupRegistration.cs`, `App/Package.appxmanifest` | Settings toggle | active |
| Keep Roblox windowed | Before every launch (with or without an FPS cap), a saved `Fullscreen=true` in Roblox's GlobalBasicSettings is turned off — covers the alt+enter clean-exit save and the crash path F-109's restore can't reach. Default on; Settings › Startup toggle. | `Core/GlobalBasicSettingsWriter.cs`, `Core/RobloxLauncher.cs`, `App/Preferences/SettingsPage.xaml` | `settings:LaunchWindowed` (default true) | active |
