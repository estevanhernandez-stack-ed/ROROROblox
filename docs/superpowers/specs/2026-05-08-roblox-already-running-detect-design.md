...
>
> **The premise of this spec turned out to be wrong.** Build-time smoke testing found the actual root cause of the "every Launch As opens the same account" bug, which is documented below as Roblox-already-running. It is **not** Roblox-already-running. It is a cookie-container leak in `IRobloxApi`'s HttpClient: `AddHttpClient<IRobloxApi, RobloxApi>` was registering with the default handler (`UseCookies=true`), so the first auth-ticket request leaked its account's `.ROBLOSECURITY` into the shared `CookieContainer`, and every subsequent Launch As routed through that cached cookie. Roblox issued the same user identity for every alt, server-side same-account-twice enforcement (Error 270) kicked the duplicate. This is documented in memory `project_rororo_v1.2_cookie_container.md` as load-bearing v1.2 config that had silently disappeared from main.
>
> **The fix that actually resolved the bug** is `.ConfigurePrimaryHttpMessageHandler(() => new SocketsHttpHandler { UseCookies = false })` on the `IRobloxApi` HttpClient registration in `App.xaml.cs:ConfigureServices`. Single line plus a comment. Verified by user 2026-05-08: with the fix, two different alts open as two different users. Without it, both alts route to the same user even with mutex held cleanly and no foreign Roblox running.
>
> **Cycle 4's gate is real defense-in-depth, not the fix.** During smoke testing the gate fired correctly twice on what the user thought was a clean cold-start (PIDs 58064 and 30060) — there were genuine residual `RobloxPlayerBeta.exe` processes the user couldn't see in the foreground but were left over from earlier sessions. The gate prevents a real broken state when those residuals are present (Roblox's roblox-player URI handler routing through the existing process); the cookie-container fix prevents the every-alt-as-same-user bug independently. They are two complementary fixes against two distinct broken states. Cycle 4 ships, but its narrative is "harden against residual Roblox processes," not "fix multi-instance."
>
> **Implementation drift from this spec body:**
>
> - **§4.4 location drift.** Spec said `src/ROROROblox.App/Startup/StartupGate.cs`. Actual: `src/ROROROblox.Core/Diagnostics/StartupGate.cs`. Reason: spec's express intent was "unit-testable without WPF," and the test project references Core but not App. Putting StartupGate in Core/Diagnostics next to the probe was more cohesive than the App/Startup folder (which holds run-on-login registration, an unrelated concern).
> - **§4.5 placement drift.** Spec said "between line 63 BuildServiceProvider and line 91 mutex.Acquire." Actual: after `ThemeService.ApplyAtStartup()` (line 77), before `tray = ...` resolution (line 79). Reason: `BgBrush` has to resolve before the modal paints or chrome flashes default-system colors; theme apply is the gate that resolves the brand-token brushes.
> - **§4.3 visual drift.** Heading is 18pt to match cycle-2 chrome (`RobloxNotInstalledWindow`, `DpapiCorruptWindow`), not the 20pt the spec specified. Spec internally contradicted itself ("Space Grotesk 20pt bold" + "matches the established cycle-2 modal pattern" — cycle-2 uses 18pt). Picked the visual-consistency interpretation.
> - **§4.3 icon addition.** Spec didn't specify a window icon; cycle-2 modals don't have one either, but every other RoRoRo window does. Added `Icon="/ROROROblox.App;component/Tray/Resources/tray-on.ico"` to match the rest-of-app pattern. Cycle-2 modals are unfixed; that's a separate cleanup.
> - **Diagnostic logging additions not in spec.** App.OnStartup logs `Mutex acquire at startup: name=..., acquired=...` after mutex.Acquire (was previously silent on failure — only the tray icon's Error state signaled it). StartupGate logs `no RobloxPlayerBeta.exe detected; proceeding to mutex.Acquire` on the gate-passes path (was previously silent on the happy path). Both surfaced during smoke when the user asked the load-bearing question "I don't even know if our dev version ever grabbed the mutex" and the existing logs couldn't answer.
>
