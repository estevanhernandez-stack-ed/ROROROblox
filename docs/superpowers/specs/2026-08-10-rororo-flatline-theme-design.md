> ## Archived 2026-08-10 — this is the canonical v1.17.0.0 spec
>
> Cart authored this cycle's design in `docs/spec.md` directly rather than compressing it to a
> pointer-stub, so unlike every prior cycle it had no copy under `docs/superpowers/specs/`. It was
> moved here verbatim before the next Cart run overwrote `docs/spec.md`, which is that file's
> documented lifecycle (`CLAUDE.md`: older cycles overwrite it each round).
>
> Nothing below is edited. The banner-corrections it already carries — the ten-items-not-eight block
> at the top, and the §5.3 corrections the rendered gate produced — are part of the record.
