...
3. Edit **Notes for certification** — **Partner Center only; the API cannot do this one.** `submit_write.py` never sends `notesForCertification`, and it read as empty through the API on v1.28 even after the letter was pasted, so confirm it on the Partner Center screen or not at all. Paste the fenced block from `docs/store/reviewer-letter-X.Y.Z.0.md` (the one below the single `---` marker; the file has one marker, not a pair) (the per-version reviewer letter; leads with that release's disclosure-surface change + the trademark disclaimer). This is reviewer-only, not shown to users.
4. **Fill "What's new in this version" on all ten listing rows.** Two ways, and since v1.28 the first is the one to use.

   **Through the listing console (preferred).** Commit the copy in this repo, point `apps/rororo-live/config.toml` at that commit, rebuild, then `submit_write.py plan` / `apply`. It writes all ten rows in one call, refuses anything over cap, and checks each block opens with the version. Runbook: `store-listing-console/docs/api-update-listings.md`.

   **Mixing the API and Partner Center on one submission is fine** — measured on v1.28, submission 1152921505701878932, against a rule that said it was not. Write the listings through the API, upload the packages and paste the reviewer letter in Partner Center, then commit. **Neither tool sees the other's edits until that commit.** Partner Center will show Store listings as "Unchanged" and serve the previous version's text while the API reports the previous version's packages; both views are incomplete and the commit reconciles them in about two minutes. Verify each half from the side that made it, and do not trust an API read of a submission in flight.

   **By hand**, if the console is unavailable: paste the fenced block from `docs/store/whats-new-X.Y.Z.0.md` into each row. Ten rows, seven blocks — German serves two rows and French three.

   **DO NOT SKIP — this is the public update-note Store users see, and it's the step that keeps getting left off.** It's a *different field* from Notes for certification (step 3): step 3 is private to Microsoft's reviewer; this one is public. Both must be filled every release. (Partner Center: Store listings → [language] → "What's new in this version".)
