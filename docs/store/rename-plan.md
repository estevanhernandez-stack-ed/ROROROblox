...
6. **Version.** Recommendation: bump to **1.1.2.0** if Microsoft accepts a Listing-only fix without a manifest re-upload, OR **1.2.0.0** if we re-pack the MSIX with a new DisplayName.
...
- [ ] `<Identity Name="626LabsLLC.RORORO" Version="1.1.0.0">` → bump Version to `1.1.2.0` (or `1.2.0.0` if we want a clean reset)
...
- [ ] Re-run `scripts/finalize-store-build.ps1 -Version "1.1.2.0"` to rebuild the Store MSIX with the new DisplayName
