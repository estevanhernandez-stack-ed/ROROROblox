...
>
> The fix is a rename of the user-visible product. The package's internal Identity (`626LabsLLC.RoRoRoBlox`) and Partner Center reservation can stay — Microsoft asked specifically for a Listing-side change. We bump version (1.1.0.1 minimum, 1.2.0.0 if we want a clean reset), update every user-visible surface, re-upload.
...
2. **New tagline.** Current `"Multi-Roblox Instant Generator."` is fine in **description body** (nominative use with disclaimer is permitted) but should NOT appear in display-name slots (wordmark, manifest DisplayName, About box title). Proposal:
   ...
   - Description body (Store listing long description): keep "Multi-Roblox Instant Generator" as the descriptive sub-positioning since description body has nominative-use latitude.
3. **Identity Name strategy.** Recommendation: **keep `626LabsLLC.RoRoRoBlox`** for now (Microsoft's rejection was Listing-specific, not Identity-specific; re-reserving means a new Store ID + losing the existing reservation). If Microsoft re-rejects citing Identity-side visibility, escalate to a re-reservation in a follow-up cycle.
4. **Internal data path strategy.** Recommendation: **keep `%LOCALAPPDATA%\ROROROblox\`** as the local data folder. These paths are filesystem-internal, never user-facing. Changing them would orphan existing dev/test data. The constant string `"ROROROblox"` lives on as a folder name — Microsoft cert reviewers don't see filesystem structure inside the MSIX install (it's virtualized to package LocalState by Identity Name anyway).
5. **Repo name.** Recommendation: **keep `github.com/.../ROROROblox`**. Repo renames break external links and require Pages baseurl reconfig. Internal-only artifact; doesn't surface to Store reviewers.
...
## Surfaces inventory — every place "ROROROblox" appears as a user-visible string
...
- [ ] `<Properties><DisplayName>ROROROblox</DisplayName>` → new name
- [ ] `<Properties><Description>` → revise (drop "Multi-Roblox Instant Generator" from this slot)
- [ ] `<Application uap:VisualElements DisplayName="ROROROblox"` → new name
...
- [ ] `<Identity Name="626LabsLLC.RoRoRoBlox" Version="1.1.0.0">` → bump Version to `1.1.0.1` (or `1.2.0.0` if we want a clean reset)
- [ ] `<Application Id="ROROROblox">` → leave (internal application ID, not user-visible)
...
- [ ] `MainWindow.xaml` line 7 — `Title="ROROROblox"` → new name
- [ ] `MainWindow.xaml` line 501 (TitleBar `Title=`) → new name
- [ ] `MainWindow.xaml` line 544 (header TextBlock `Text="ROROROblox"`) → new name
- [ ] `MainWindow.xaml` footer `Text="Multi-Roblox Instant Generator. A 626 Labs product."` → revise (drop the Roblox-naming half OR replace with new tagline)
- [ ] `AboutWindow.xaml` `Title="About ROROROblox"` → new name
- [ ] `AboutWindow.xaml` line 58 hero `<TextBlock Text="ROROROblox">` → new name
- [ ] `AboutWindow.xaml` line 71 tagline `<TextBlock Text="Multi-Roblox Instant Generator.">` → revise (drop or replace)
- [ ] `WelcomeWindow.xaml` line 41 `<TextBlock Text="ROROROblox">` → new name
...
- [ ] Search `*.xaml.cs` for any string literal containing `"ROROROblox"` and assess each — most are filesystem paths (keep) or window titles (change)

**Tray:**
- [ ] `TrayService.cs:67` — fallback string `"ROROROblox"` (likely a tooltip) → new name
...
- [ ] `App.xaml.cs:235, 243` — `ProductInfoHeaderValue("ROROROblox", version)` (User-Agent sent to Roblox + GitHub) → new name (consistent with brand; servers don't enforce)
- [ ] `StartupRegistration.cs:8` — `ValueName = "ROROROblox"` (visible in Task Manager → Startup Apps tab) → new name

**Keep (filesystem-internal, no user-facing surface):**
- `AppLogging.cs:19` — log dir name `"ROROROblox"` → keep (filesystem path, not a label)
- `AppLogging.cs:32` — `.WithProperty("App", "ROROROblox")` → keep (internal log property)
- `AccountStore.cs:41`, `AppSettings.cs:40`, `FavoriteGameStore.cs:39` — data path `"ROROROblox"` → keep (path stability across rename)
...
- `Package.appxmanifest` `<Application Id="ROROROblox">` → keep (internal app ID)
...
- Internal application ID: `<Application Id="ROROROblox">`
- GitHub repo URL: `github.com/estevanhernandez-stack-ed/ROROROblox`
- GitHub Pages baseurl: `/ROROROblox`
- Privacy URL: `https://estevanhernandez-stack-ed.github.io/ROROROblox/privacy/`
- Identity Name: `626LabsLLC.RoRoRoBlox` (unless Microsoft escalates the rejection)
...
| **Pages URL breaks** if baseurl changes | We're keeping `/ROROROblox` as the baseurl. |
| **Tagline rewrite degrades brand voice** | Keep "Multi-Roblox Instant Generator" in the description body (nominative-use defense already established); only remove from display-name surfaces. |
